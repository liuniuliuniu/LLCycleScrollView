...
protocol LLCycleScrollViewDelagate: class {
    func LLCycleScrollViewImageSelectedIndex(index:Int)
}


enum LLCycleScrollViewPageContolAliment:Int{
    case LLCycleScrollViewPageContolAlimentRight
    case LLCycleScrollViewPageContolAlimentCenter
}
let collectionViewCellID = "collectionViewCellID"

class LLCycleScrollView: UIView {
    
    /// 闭包:回调图片的index
    var callBackWithIndex:((Int)->())?    
    /// 代理
    weak open var delegate: LLCycleScrollViewDelagate?
    /// 文字标题数组
    ...
    var pageControlAliment = LLCycleScrollViewPageContolAliment(rawValue: 1)
    /// 是否显示分页控件
    ...
    /// 网络图片数组
    private  var imageURLStringsGroup = NSArray(){
        didSet{
    ...
    /// 本地图片数组
    private  var localizationImagesGroup = NSArray(){
        didSet{
            self.imagesGroup = NSMutableArray.init(array: localizationImagesGroup)
        }
    }
    
    private var imagesGroup = NSMutableArray.init(){
        didSet{
    ...
    /// 初始化方法1 本地图片
    class  func cycleScrollView(frame : CGRect,imagesGroup : NSArray) -> LLCycleScrollView {
        let cycleScrollView = LLCycleScrollView.init(frame: frame)
        cycleScrollView.imagesGroup = NSMutableArray.init(array: imagesGroup)
        return cycleScrollView
    }
    /// 初始化方法2 网络图片
    class  func cycleScrollView(frame : CGRect,imageURLGroup : NSArray) -> LLCycleScrollView {
        let cycleScrollView = LLCycleScrollView.init(frame: frame)
...
extension LLCycleScrollView{
    ...
    fileprivate func initConfiguration() {
        pageControlAliment = LLCycleScrollViewPageContolAliment.LLCycleScrollViewPageContolAlimentCenter
    ...
    fileprivate func setupMainView() {
        ...
        mainView?.register(LLCollectionViewCell.self, forCellWithReuseIdentifier: collectionViewCellID)
    ...
    override func layoutSubviews() {
        ...
        let size = pageControl?.size(forNumberOfPages:imagesGroup.count)
        var x = (self.bounds.size.width - (size?.width)!) * 0.5
        if pageControlAliment == LLCycleScrollViewPageContolAliment.LLCycleScrollViewPageContolAlimentRight {
            x = (self.mainView?.bounds.size.width)! - (size?.width)! - 10
        }
        let y = (self.mainView?.bounds.size.height)! - (size?.height)!
...
extension LLCycleScrollView:UICollectionViewDelegate,UICollectionViewDataSource{
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: collectionViewCellID, for: indexPath) as! LLCollectionViewCell
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        if self.delegate != nil {
            self.delegate?.LLCycleScrollViewImageSelectedIndex(index: indexPath.item % imagesGroup.count)
        }
    }
